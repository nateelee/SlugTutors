...
def get_full_name():
    return (
        f"{auth.current_user.get('first_name')} {auth.current_user.get('last_name')}"
        if auth.current_user
        else None
    )
...
db.define_table(
    "tutors",
    Field("user_id", "reference auth_user", writable=False, readable=False, default=get_user),
    ...
    Field("thumbnail", "upload", label="Avatar", uploadfield="thumbnail_blob"),
    Field("thumbnail_blob", "blob"),
    Field("Monday"),
    Field("Tuesday"),
    Field("Wednesday"),
    Field("Thursday"),
...
db.define_table(
    ...
    Field("rater_name"),
    Field(
        "rater_id", "reference auth_user", default=get_user
